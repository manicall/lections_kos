#include <Windows.h>
#include <iostream>
using namespace std;

wchar_t ServiceName[] = L"longstartsvc";
enum { CREATESERVICE, STARTSERVICE, DELETESERVICE };
int getAnswer() {
    cout << "===========MENU===========" << endl;
    cout << "1) —оздать сервис" << endl;
    cout << "2) «апустить сервис" << endl;
    cout << "3) ”далить сервис" << endl;
    int a; cin >> a;
    return a - 1;
}
void MyErrorExit(const char* msg) {
    cout << msg << endl;
    cout << " од ошибки: " <<GetLastError() << endl;
    system("pause");
    ExitProcess(1);
}

VOID CreateSampleService(SC_HANDLE schSCManager, SC_HANDLE& schService);
VOID DeleteSampleService(SC_HANDLE schSCManager, SC_HANDLE& schService);
DWORD StartSampleService(SC_HANDLE schSCManager, SC_HANDLE& schService);

int main() {
    setlocale(LC_ALL, "ru");
    SC_HANDLE schSCManager, schService;
    schSCManager = OpenSCManager(
        NULL,                    // локальна€ машина
        NULL,                    // им€ базы (ServicesActive)
        SC_MANAGER_ALL_ACCESS);  // полный доступ
    while (true) {
        switch (getAnswer()) {
        case CREATESERVICE: CreateSampleService(schSCManager, schService); break;
        case STARTSERVICE: StartSampleService(schSCManager, schService);  break;
        case DELETESERVICE: DeleteSampleService(schSCManager, schService); break;
        default: cout << "Ќеизвестна€ команда, программа будет завершена. "; system("pause"); return 2;
        }
    }
    system("pause");
}

VOID CreateSampleService(SC_HANDLE schSCManager, SC_HANDLE& schService)
{
    

    if (schSCManager == NULL)
        MyErrorExit("OpenSCManager");

    LPCTSTR lpszBinaryPathName = L"C:\\Users\\max\\Desktop\\LD_service.exe";
    LPCTSTR lpszDisplayName;
    schService = CreateService(
        schSCManager,              // база данныйх SCManager
        L"Sample_Srv",             // им€ сервиса
        ServiceName,               // им€, которое будет показыватьс€
        SERVICE_ALL_ACCESS,        // уровень доступа
        SERVICE_WIN32_OWN_PROCESS, // сервис будет запускатьс€
                                   // в собственном процессе
        SERVICE_DEMAND_START,      // будет запускатьс€ "вручную"
        SERVICE_ERROR_NORMAL,      // стандартный режим
                                   // протоколировани€ ошибок
        lpszBinaryPathName,        // путь к сервису
        NULL,                      // сервис не принадлежит группе
        NULL,                      // --""--
        NULL,                      // нет зависимости
        NULL,                      // — правами системы
        NULL);                     // без парол€

    if (schService == NULL)
        MyErrorExit("CreateService");
    else
        printf("CreateService SUCCESS.\n");

    CloseServiceHandle(schService);
}

VOID DeleteSampleService(SC_HANDLE schSCManager, SC_HANDLE& schService)
{
    schSCManager = OpenSCManager(
        NULL,                    // локальна€ машина
        NULL,                    // им€ базы (ServicesActive)
        SC_MANAGER_ALL_ACCESS);  // полный доступ
    if (schSCManager == NULL)
        MyErrorExit("OpenSCManager");
    schService = OpenService(
        schSCManager,       // база данных SCManager
        L"Sample_Srv",      // им€ сервиса
        DELETE);            // нужно только удалить
    if (schService == NULL)
        MyErrorExit("OpenService");

    if (!DeleteService(schService))
        MyErrorExit("DeleteService");
    else
        printf("DeleteService SUCCESS\n");
    CloseServiceHandle(schService);
}

DWORD StartSampleService(SC_HANDLE schSCManager, SC_HANDLE& schService)
{
    SERVICE_STATUS ssStatus;
    DWORD dwOldCheckPoint;
    DWORD dwStartTickCount;
    DWORD dwWaitTime;
    DWORD dwStatus;

    schService = OpenService(
        schSCManager,           // база сервисов SCM
        L"Sample_Srv",          // им€ сервиса
        SERVICE_ALL_ACCESS);

    if (schService == NULL)
    {
        MyErrorExit("OpenService");
    }

    if (!StartService(
        schService,  // дескриптор сервиса
        0,           // количество аргументов
        NULL))       // нет аргументов
    {
        MyErrorExit("StartService");
    }
    else
    {
        printf("Service start pending.\n");
    }

    // ѕровер€ем состо€ние сервиса до тех пор, пока он не запуститс€.

    if (!QueryServiceStatus(
        schService,   // дескриптор сервиса
        &ssStatus))  // адрес структуры с информацией о сервисе
    {
        MyErrorExit("QueryServiceStatus");
    }

    // —охран€ем счЄтчик тиков и начальную точку.

    dwStartTickCount = GetTickCount();
    dwOldCheckPoint = ssStatus.dwCheckPoint;

    while (ssStatus.dwCurrentState == SERVICE_START_PENDING)
    {
        // Ќе нужно ожидать дольше, чем приблизительное врем€,
        // необходимое дл€ старта. ќбычно ожидать лучше всего одну
        // дес€тую этого значени€, но не более одно секунды
        // и никак не более 10 секунд.

        dwWaitTime = ssStatus.dwWaitHint / 10;

        if (dwWaitTime < 1000)
            dwWaitTime = 1000;
        else if (dwWaitTime > 10000)
            dwWaitTime = 10000;

        Sleep(dwWaitTime);

        // —нова провер€ем статус.

        if (!QueryServiceStatus(
            schService,   // дескриптор сервиса
            &ssStatus))  // адрес структуры
            break;

        if (ssStatus.dwCheckPoint > dwOldCheckPoint)
        {
            // —ервис в процесса старта.

            dwStartTickCount = GetTickCount();
                dwOldCheckPoint = ssStatus.dwCheckPoint;
        }
        else
        {
            if (GetTickCount() - dwStartTickCount > ssStatus.dwWaitHint)
            {
                // ¬рем€ до старта не изменилось
                break;
            }
        }
    }

    if (ssStatus.dwCurrentState == SERVICE_RUNNING)
    {
        printf("StartService SUCCESS.\n");
        dwStatus = NO_ERROR;
    }
    else
    {
        printf("\n—ервис на запущен. \n");
        printf("  Current State: %d\n", ssStatus.dwCurrentState);
        printf("  Exit Code: %d\n", ssStatus.dwWin32ExitCode);
        printf("  Service Specific Exit Code: %d\n",
            ssStatus.dwServiceSpecificExitCode);
        printf("  Check Point: %d\n", ssStatus.dwCheckPoint);
        printf("  Wait Hint: %d\n", ssStatus.dwWaitHint);
        dwStatus = GetLastError();
    }

    CloseServiceHandle(schService);
    return dwStatus;
}

